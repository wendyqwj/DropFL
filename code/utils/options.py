...
def args_parser():
    ...
    parser.add_argument('--alpha', type=float, default=0.3,
                        help="the fraction of users that Attackers control")
    parser.add_argument('--delta', type=float, default=0.5,
                        help="为攻击掉线数量占控制节点的比率")
    parser.add_argument('--record_round', type=int,
                        default=5, help="每训练多少轮, 记录一次数据至log")
    parser.add_argument('--P', type=float, default=1,
                        help="The probability that a label is assigned to the correct group")
    parser.add_argument('--MAX_WORKERS', type=int,
                        default=10, help="多线程程度, 太大的话线程会炸")
    ...
    parser.add_argument('--defend', action='store_true',
                        help='whether use defend mode or not')
    parser.add_argument('--detect', action='store_true',
                        help='whether use detect mode or not')
